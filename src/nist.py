import copy, csv, pydot
import pylab, re, logging

from kegg import Kegg
from kegg_errors import KeggParseException,\
    KeggReactionNotBalancedException
from thermodynamics import default_T, MissingCompoundFormationEnergy,\
    PsuedoisomerTableThermodynamics
from util import _mkdir, calc_rmse
from html_writer import HtmlWriter
from thermodynamic_constants import R
from kegg_reaction import Reaction
from database import SqliteDatabase
from plotting import binned_plot
from thermodynamic_errors import MissingReactionEnergy
...
class NistRowData:
    ...
    def ReadFromDatabase(self, name, row_dict):
        ...
        # is effectively [Mg] = 0).
        self.I = NistRowData.none_float(row_dict['I']) or 0.1
...
class Nist(object):
    ...
    def verify_results(self, html_writer, thermodynamics, name=None):
        ...
        for row in total_list: 
            unique_reaction_dict.setdefault(row[3], []).append(row[0])
        ...
        fig = pylab.figure(figsize=(4.0, 4.0), dpi=90)
        pylab.hold(True)
        ...
        pylab.text(-50, 40, r'RMSE = %.1f [kJ/mol]' % (unique_rmse), fontsize=12)
        pylab.xlabel(r"observed $\Delta_r G'^\circ$ [kJ/mol]", fontsize=14)
        pylab.ylabel(r"estimated $\Delta_r G'^\circ$ [kJ/mol]", fontsize=14)
        #min_x = min(dG0_obs_vec)
        #max_x = max(dG0_obs_vec)
        pylab.plot([-70, 70], [-70, 70], 'k--')
        pylab.axis([-70, 70, -70, 70])
        if name:
        ...
        for row in sorted(total_list, reverse=True):
            ...
            d['_reaction'] = row[3].FullReactionString(show_cids=False)
            ...
            d['eval.'] = row[8]
            if row[9]:
                d['url'] = '<a href="%s">%s</a>' % (row[9], row[10])
                d['_url'] = row[9]
            else:
                d['url'] = ''
                d['_url'] = ''
            d['NIST reference'] = row[10]
            dict_list.append(d)
        html_writer.write_table(dict_list, table_headers)
        
        csv_fname = html_writer.relative_to_full_path(name+"_data.csv")
        fieldnames = ['|err|', 'dG\'0 (obs)', 'dG\'0 (est)',
                      'pH', 'I', 'T', 'NIST reference', '_reaction', '_url']
        csv_out = csv.DictWriter(open(csv_fname, 'w'),
                                 fieldnames=fieldnames,
                                 extrasaction='ignore')
        csv_out.writeheader()
        csv_out.writerows(dict_list)
