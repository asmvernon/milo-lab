...
from group_decomposition import GroupDecomposer
from thermodynamics import Thermodynamics,\
    MissingCompoundFormationEnergy, PsuedoisomerTableThermodynamics
from pseudoisomer import PseudoisomerMap
from thermodynamic_constants import default_I, default_pMg, default_T,\
    default_pH
from linear_regression import LinearRegression
from database import SqliteDatabase
from html_writer import HtmlWriter, NullHtmlWriter
from util import _mkdir
from sparse_kernel import SparseKernel
...
class NistRegression(PsuedoisomerTableThermodynamics):
    ...
    def ReverseTransform(self, anchors=None, cid2nH_nMg=None):
        ...
        for i in xrange(n_unique_rows):
            ...
            unique_data_mat[i, 0:2] = np.median(sub_data_mat, 0)
    ...
    def WriteUniqueReactionReport(self, unique_sparse_reactions, 
        ...
        logging.info('unique std(dG0_r) = %.1f' % np.mean(unique_data_mat[:, 2]))
        logging.info('unique std(dG\'0_r) = %.1f' % np.mean(unique_data_mat[:, 3]))
